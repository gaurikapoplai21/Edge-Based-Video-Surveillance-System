...
class face_detect:
	...
	def detect_mask(self, only_face, only_face_color):
		...
		x_ = len(only_face);
		y_ = len(only_face[0]);
		mask = only_face[int(x_*per):x_-int(x_*0.1), int(y_*0.3):y_-int(y_*0.3)]
		...
		if (white/total)*100 > 99 or (black/total)*100 > 99:
			print("Nice mask")
			cv2.imwrite('E:/Capstone/Implementation/check/mmsk/'+str(self.imgno)+'.png', only_face_color)
		else:
			print("No mask")
			cv2.imwrite('E:/Capstone/Implementation/check/nmsk/'+str(self.imgno)+'.png', only_face_color)
		self.imgno += 1;
		return only_face
...
if __name__ == '__main__':
	...
	while 1:
		ret, frame = cap.read()
		frame = f.main(frame);
		if len(frame): cv2.imshow('Mask Detection', frame)
		else: break
